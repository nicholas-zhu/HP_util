function [roi_fit, roi_SNR] = dynamic_biexp_adc(img_ref,img_arr,b_arr,FAC_arr,mask,adc_range,noise_level,reg_flag)
...
% mask
if isempty(mask)||sum(size(mask)~=im_size)
    figure;
    mask1 = roipoly(HImg);
...
img_arr = abs(img_arr);
img_scale = mean(img_arr(:));
img_arr = img_arr/img_scale;
m_img = mask.*img_arr;
roi_adc = zeros([im_size,nb,Dyn_N]);
...
for i = 1:Dyn_N
    for k = 1:length(ind_x)
        ...
        if nb < 4
            [x, R2t] = biexp_fit3(b_arr,dwi_pixel,noise_level.*vec(FAC_arr),adc_range,0);
        else
            [x, R2t] = biexp_fit(b_arr,dwi_pixel,noise_level.*vec(FAC_arr),adc_range,0);
        end
...
function [x, R2] = biexp_fit3(bval,dwi,noise,bound,power_flag)
% bi-exponential fitting
%   S_i = S1*exp(-b*ADC1)+S2*exp(-b*ADC2)
% Input:
%   bval:   b-val [N,1]
%   dwi:    diffusion signal [N,1]
%   noise:  noise [N,1]
%   bound:  bound of Variables x [3,2] S1,S2,2e-3,ADC2
%   power_flag: power method
% Output:
%   x:      fitting result
%   R2:     R square for fitting
...
if power_flag == 1
    S = S.^2-noise.^2;
    afun = @(x,b)(x(1)*exp(-b*ADC_in)+x(2)*exp(-b*x(3))).^2;
else
    afun = @(x,b)(x(1)*exp(-b*ADC_in)+x(2)*exp(-b*x(3)));
end
x0 = [S(1)/2,S(1)/2,1e-3];
[x,rss] = lsqcurvefit(afun,double(x0),double(bval),double(S),double(bound(:,1)),double(bound(:,2)));
